...
def poll(image_path, scores_from_ground, span, alpha=):
    ...
    clip = clip.to(device)
    clip.load_state_dict(torch.load('/path/to/save_pope_clip/best.pt'))
...
def eval_one_sentence(sent, img_path, model, processor):
...
if __name__ == "__main__":
    ...
    if dataset_name == "realworldqa":
        with open('/path/to/jsonl'', 'r') as f:
            data = f.readlines()
        #data = data + data2
        df1 = pd.read_parquet('/path/to/realworldqa/data/test-00000-of-00002.parquet')
        df2 = pd.read_parquet('/path/to/realworldqa/data/test-00001-of-00002.parquet')
